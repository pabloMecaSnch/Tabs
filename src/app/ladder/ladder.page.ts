...
ladder:any[];
  ...
  constructor(public navCtrl:NavController) {
   ...
   reorderItems(indexes){
     //this.ladder = reoderArray(this.ladder , indexes);
     console.log(indexes);
   }

   onRenderItems(event){
     console.log(`Move item from ${event.detail.from} to ${event.detail.to}`);
     const dreaggedItem = this.ladder.splice(event.detail.from,1)[0];
     this.ladder.splice(event.detail.to,0,dreaggedItem);
